...
1. **1차시 ① 생각 나누기 → 1차시 ② 발견 · 언어 데이터 수집**: 교사가 주제를 입력하고 생각 나누기를 시작하면 학생이 같은 1차시 QR에서 찬성·반대와 짧은 이유를 제출합니다. 교사가 발견 단계로 전환하면 학생 화면이 자동으로 바뀌고 `비속어·유행어·외래어`를 모읍니다. 같은 표현은 새 행을 만들지 않고 등록 횟수를 올립니다.
